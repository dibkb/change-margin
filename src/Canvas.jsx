...
const Canvas = () => {
  ...
  useEffect(() => {
    ...
    canvas.style.height = `${divRef.current.getBoundingClientRect().height}px`;
    canvas.style.width = `${divRef.current.getBoundingClientRect().width}px`;
  }, []);
  useEffect(() => {
    const canvas = canvasRef.current;
    const context = canvas.getContext("2d");
    divRef.current.style.top = `${marginTop}px`;
    let heightToIncrease =
      divRef.current.getBoundingClientRect().bottom -
      canvasRef.current.getBoundingClientRect().bottom;
    canvasRef.current.style.height = `${
      canvasRef.current.getBoundingClientRect().bottom + heightToIncrease
    }px`;
